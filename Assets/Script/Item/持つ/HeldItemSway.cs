...
public class HeldItemSway : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        float vertical =
            Mathf.Clamp(-move.y * 0.01f, -verticalAmount, verticalAmount);

        targetPosition.y += vertical;

        currentPosition =
            Vector3.Lerp(currentPosition, targetPosition, Time.deltaTime * smooth);

        currentRotation =
            Vector3.Lerp(currentRotation, targetRotation, Time.deltaTime * smooth);
